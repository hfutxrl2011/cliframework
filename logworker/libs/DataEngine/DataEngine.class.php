...
{	
	...
	public function preCheckTask(){
		...
		if( isset($ret[0]['count']) && $ret[0]['count'] >= 1   ) {
			$this->updateStatus('0',false,'give up do this task');
		}
		/*
	...
	public function flowCheck(){
		if(!empty(Conf::$flowControl['app_id']) 
			&& in_array($this->_data['app_id'],Conf::$flowControl['app_id'])
			&& Conf::$packOS == Conf::$flowControl['packos']
		){
			if(in_array(Conf::$packSvr,Conf::$flowControl['packsvr'])){
				return true;
			}else{
	...
	public function getAppKey(){
		$sql = "select app_key,api_url from dev_app_info where app_id='".$this->_data['app_id']."'";
		$ret = $this->query($sql);
		if( isset($ret[0]) && !empty($ret[0]) ) {
	...
	public function updateStatus($status,$result=true,$desc=null){
		if(null == $desc){
