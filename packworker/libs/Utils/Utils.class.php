...
class Utils {
	...
	public static function initEnv(){
		...
		$afterData['jpush_app_key'] = isset($beforeData['taskInfo']['jpush_app_key']) ? $beforeData['taskInfo']['jpush_app_key'] : 'b7bf5ec527d73136c7a0944c';
		$afterData['bbsname'] = isset($beforeData['taskInfo']['bbs_name']) ? $beforeData['taskInfo']['bbs_name'] : 'BigApp';
		...
		$afterData['app'] = $beforeData['app'];
		$afterData['app_id'] = $beforeData['app_id'];
		$afterData['task_type'] = $beforeData['task_type'];
		$afterData['inner_version'] = isset($beforeData['taskInfo']['inner_version']) ? $beforeData['taskInfo']['inner_version'] : '';
		$afterData['outer_version'] = isset($beforeData['taskInfo']['outer_version']) ? $beforeData['taskInfo']['outer_version'] : '';
	...
	public static function thumbIMG($width,$height,$imgpath,$srcimgpath=null){
		require_once(dirname(dirname(__FILE__)).'/Image/Image.class.php');
		$ret = true;
		$gd_info  = 0;
		...
		if($gd_info === 0){
			...
			if(@$Image->get_handle())
			{
				@$Image->open($srcimgpath);				
				@$Image->thumb($width,$height)->save($imgpath);//缩放图片
			}
			$ret = file_exists($imgpath);
		}
		return $ret;
	}
	
	public static function sendEmail($subject,$content,$to,$from='icafe@uuzu.com',$tpl = 'template1.html'){
		...
		$template = dirname(__FILE__).'/../PHPMailer/template/'.$tpl;
		if(file_exists($template)){
			$contents = file_get_contents($template);
			$contents = str_replace('{subjects}',$subject,$contents);
