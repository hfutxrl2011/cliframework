...
#sourceAppPath="/Users/youzu/Desktop/IOSPACKAGE/IOS"
...
#defaults write $rootpath/$normalpath/${TARGET}/Info.plist CFBundleIdentifier "${bundleid}"
defaults read $rootpath/$normalpath/${TARGET}/Info.plist
#exit
#输出ThemeStyele.plist中所有信息
#defaults read $rootpath/$normalpath/Clan/Resource/ThemeStyle.plist
#改变某个key的value
#生产环境 要把OfflineMode关闭 设置为0
defaults write ${rootpath}/$normalpath/${TARGET}/Resource/ThemeStyle.plist OfflineMode "0"
...
#security unlock-keychain -p 111111 ${rootpath}/Library/Keychains/login.keychain
#
##script -q -t 0 $rootpath/Desktop/ximi/package/$name/archive.log xcodebuild archive -workspace "${XCP_PATH}" -scheme "${TARGET}" -archivePath "${ARCHIVE_PATH}" CODE_SIGN_IDENTITY="${CODE_SIGNING_IDENTITY}" PROVISIONING_PROFILE="${PROVISIONING_PROFILE}"
#
##echo ">>>>>>>">/Users/youzu/Desktop/ximi/exportaaa.log
#security unlock-keychain -p 111111 ${rootpath}/Library/Keychains/login.keychain
#script -q -t 0 $rootpath/${basepath}/$name/exportapp.log xcodebuild -exportArchive -exportFormat ipa -archivePath "${ARCHIVE_PATH}" -exportPath "${EXPORT_PATH}" -exportProvisioningProfile "${provisionname}"
#security unlock-keychain -p 111111 ${rootpath}/Library/Keychains/login.keychain

#script -q -t 0 $rootpath/${basepath}/$name/archive.log xcodebuild archive -workspace "${XCP_PATH}" -scheme "${TARGET}" -archivePath "${ARCHIVE_PATH}" CODE_SIGN_IDENTITY="${CODE_SIGNING_IDENTITY}" PROVISIONING_PROFILE="${PROVISIONING_PROFILE}"
#script -q -t 0 $rootpath/${basepath}/$name/archive2.log xcodebuild -exportArchive -exportFormat ipa -archivePath "${ARCHIVE_PATH}" -exportPath "${EXPORT_PATH}" -exportProvisioningProfile "${provisionname}"
#script -q -t 0 $rootpath/${basepath}/$name/archive3.log node /Users/zhangsh/zhengtao/packageios/scp.js $1 $2


#echo "xcodebuild -exportArchive -exportFormat ipa -archivePath \"${ARCHIVE_PATH}\" -exportPath \"${EXPORT_PATH}\" -exportProvisioningProfile \"${provisionname}\"">>/Users/youzu/Desktop/ximi/exportaaa.log
#清除掉拷贝的源代码目录 后期可以加上
#rm -rf $rootpath/$normalpath
#fi
