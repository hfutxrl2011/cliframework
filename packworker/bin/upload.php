 ...
 * @create: 2015-07-23 01:20:58
 * @modify: 2015-07-23 01:20:58
 * @brief:  打包服务在完成打包后，由此脚本将打包后的文件
 *          上传至文件服务器，并更新db          
 ***********************************************************/
...
*/
    ...
    private function procTask($dirname)
    {
        ...
        if (count($arr)>1) {return;}  //!< online目录不要处理
        $taskid = $dirname;
...
//exit(0);
    ...
    {/*{{{*/
        ...
        $task_work_path = array (
            ...
            ///////////////////////////////////////
            "ios_online" => array (
                "monitor_file" => $this->_monitor_path."/".$taskid."/".$taskid."_online.ipa",
                "local_path"   => $root."/".$fmap["ios"]."/$pfix",
            ...
            "iosplist_online" => array (
                "monitor_file" => $this->_monitor_path."/".$taskid."/".$taskid."_online.plist",
                "local_path"   => $root."/".$fmap["iosplist"]."/$pfix",
        ...
        $keys = array("ios_tar", "ios", "iosplist");
        if ($this->_os == 1) {
    ...
    {/*{{{*/
        if (!Conf::$uploadConf["save_work_path"]) {
            $work_taskdir = $this->_work_path."/".$taskid;
